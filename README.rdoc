...
resque-status <b>requires Redis >= 1.1</b> (though I recommend getting the latest stable version).
...
itself this removes the dependency on a Redis, however, <tt>Resque::Status</tt> uses Redis
