...
Support for resque-lock is built-in. If you extend JobWithStatus, then provide
an instance method called <tt>lock</tt> just like <tt>perform</tt>.

  class SleepJobLock < Resque::JobWithStatus
    extend Resque::Plugins::Lock

    def lock
      return "lock:sleep_job-#{options['length'].to_i || 1000}"
    end
...
When calling <tt>create</tt> on these types of jobs, the return value will be nil instead of a uuid
if a job with the same lock value already is queued.
